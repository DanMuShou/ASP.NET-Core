...
) : IRequestHandler<AddUserCommand, ServiceResponse<UserDto>>
{
    ...
    )
    {
        ...
        {
            logger.LogError(ErrorMessageStr.EmailExist);
            return ServiceResponse<UserDto>.Return409(ErrorMessageStr.EmailExist);
        }

        var user = mapper.Map<Data.Entities.User>(request);
        user.UserName = request.Email;
        user.Id = Guid.NewGuid();
        user.CreatedBy = userInfoToken.Id;
        ...
        await claimsHelper.AddUserClaim(userManager, user, request.UserClaims, logger); // 为用户添加权限声明
        ...
        var passwordResult = await userManager.ResetPasswordAsync(user, token, request.Password); //它为指定用户生成一个一次性使用的安全令牌, 这个令牌用于后续的密码设置或重置操作, 可以安全地为用户设置初始密码或重置现有密码
